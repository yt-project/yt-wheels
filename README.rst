################################
Building and uploading yt wheels
################################
...
https://travis-ci.org/yt-project/yt-wheels

Appveyor interface at
https://ci.appveyor.com/project/yt-project/yt-wheels

The driving github repository is
https://github.com/yt-project/yt-wheels
...
* builds a yt wheel, linking against these fresh builds;
...
directory pointed to by http://wheels.scipy.org. Nathan Goldbaum contacted
Matthew Brett in February 2018 to get the encryption key used in this
repository.
...
* making a commit to the `yt-wheels` repository (e.g. with `git
...
Which yt commit does the repository build?
===============================================

The `yt-wheels` repository will build the commit specified in the
...
be something like::
    ...
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t macosx yt $VERSION
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t manylinux1 yt $VERSION
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t win yt $VERSION
...
`yt` is the root name of the wheel(s) to download / upload and `3.4.1` is
...
`yt-3.4.1-` from the URL in ``$CDN_URL`` above to ``~/wheelhouse``, then
upload them to pypi.
